...
import model_landmark_cal_pb2
import model_landmark_cal_pb2_grpc
...
class Model_Landmark_CalculatorServicer(model_landmark_cal_pb2_grpc.Model_Landmark_CalculatorServicer):
    ...
    def Add(self, request, context):
        imgBytes, final = Model_Landmark_CalculatorServicer().DeepFACE(request.model_landmark_idata)
        ################### CLIENT % Redis
        """
        redis_data = {
        ...
        if r.json().get(str(hashlib.sha256(imgBytes).hexdigest()))!=None: #If the model-age-gender has finished its job
        ...
        ################## END
        return model_landmark_cal_pb2.Model_Landmark_Output_img(model_landmark_odata=final) #The odata would be printed on the stdout of the first client

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    model_landmark_cal_pb2_grpc.add_Model_Landmark_CalculatorServicer_to_server(Model_Landmark_CalculatorServicer(), server)
